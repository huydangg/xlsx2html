...
char *int_to_column_name(unsigned int n) {
  char *column_name = (char *)XML_Char_malloc(4);
  column_name[0] = '\0';
  while (n > 0) {
    n--;
...
// determine row number based on cell coordinate (e.g. "A1"), returns 1-based
// row number or 0 on error
...
unsigned short column_name_to_number(const char *column_name) {
  ...
  int i, j, result = 0;
  for (i = 0, j = XML_Char_len(col_name) - 1; i < XML_Char_len(col_name);
       i += 1, j -= 1) {
...
int generate_columns(struct ArrayCols array_cols, unsigned short end_col_number,
                     unsigned short index_worksheet) {
  ...
  char *THE_FIRST_CHUNK_PATH =
      (char *)XML_Char_malloc(LEN_THE_FIRST_CHUNK_PATH + 1);
  snprintf(THE_FIRST_CHUNK_PATH, LEN_THE_FIRST_CHUNK_PATH + 1,
           "%s/chunk_%d_0.chunk", CHUNKS_DIR_PATH, index_worksheet);
  FILE *fchunk0;
  ...
  if (array_cols.length != 0) {
    for (int i = 1; i <= end_col_number; i++) {
      ...
      for (int index_col = 0; index_col < array_cols.length; index_col++) {
        if (i >= array_cols.cols[index_col]->min &&
            i <= array_cols.cols[index_col]->max) {
          float column_width_in_px =
              array_cols.cols[index_col]->width * (64 * 1.0 / 8.43);
          snprintf(TH_STRING, sizeof(TH_STRING), "<th style=\"width:%gpx;\">",
                   column_width_in_px);
          break;
        }
      }
...
void _generate_cells(unsigned int current_row, unsigned short max_col,
                     unsigned short index_current_sheet, FILE *_worksheet_file,
  ...
  for (int i = 1; i <= max_col; i++) {
    ...
    char *td = (char *)XML_Char_malloc(len_td_str + 1);
    snprintf(td, len_td_str + 1,
             "<td id=\"%d_%s%d\" style=\"border-style:hidden;\"></td>",
             index_current_sheet, col_name, current_row);
    fputs(td, _worksheet_file);
    free(col_name);
    free(td);
  }
...
void generate_cells(void *callbackdata, const XML_Char *name) {
  if (XML_Char_icmp(name, "row") == 0) {
    struct WorkSheet *worksheet_callbackdata = (struct WorkSheet *)callbackdata;
    _generate_cells(worksheet_callbackdata->ROW_NUMBER,
                    worksheet_callbackdata->end_col_number, INDEX_CURRENT_SHEET,
                    worksheet_file, &worksheet_callbackdata->num_of_chunks);
  }
  XML_SetElementHandler(xmlparser, col_row_start_element,
                        worksheet_end_element);
}

void get_end_col_end_row_from_range(const XML_Char *range, char **end_row,
                                    char **end_col) {
  // ex: A1:Q109 or I28
  int pos_colon = 0;
  char *check_colon = (char *)strchr(range, ':');
  if (check_colon) {
  ...
  while (*_tmp_end_cell) {
    if (*_tmp_end_cell >= 'A' && *_tmp_end_cell <= 'Z') {
      ...
      *end_col = (char *)XML_Char_malloc((count_col_char + 1) * sizeof(char));
      memcpy(*end_col, _tmp_end_col, (count_col_char + 1) * sizeof(char));
      *end_row =
          (char *)XML_Char_malloc(sizeof(char) * (length - count_col_char + 1));
      memcpy(*end_row, _tmp_end_cell,
             (length - count_col_char + 1) * sizeof(char));
      return;
    }
    _tmp_end_cell++;
  }
...
void worksheet_start_element(void *callbackdata, const XML_Char *name,
  ...
  if (XML_Char_icmp(name, "worksheet") == 0) {
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "ref") == 0) {
        get_end_col_end_row_from_range(attrs[i + 1], &_tmp_end_row,
                                       &_tmp_end_col);
        worksheet_callbackdata->end_row = _tmp_end_row;
        worksheet_callbackdata->end_col = _tmp_end_col;
        worksheet_callbackdata->end_col_number =
            column_name_to_number(_tmp_end_col);
        break;
      }
    }
    ...
    worksheet_callbackdata->array_cols.cols =
        (struct Col **)XML_Char_malloc(sizeof(struct Col *));
    ...
    if (worksheet_callbackdata->has_cols == '0') {
      ...
      int status = generate_columns(worksheet_callbackdata->array_cols,
                                    worksheet_callbackdata->end_col_number,
                                    INDEX_CURRENT_SHEET);
      if (status == -1) {
    ...
    int LEN_CHUNKS_DIR_PATH =
        XML_Char_len(OUTPUT_DIR) + XML_Char_len(CHUNKS_DIR_NAME) + 1 + 1;
    char *CHUNKS_DIR_PATH = (char *)XML_Char_malloc(LEN_CHUNKS_DIR_PATH);
    snprintf(CHUNKS_DIR_PATH, LEN_CHUNKS_DIR_PATH, "%s/%s", OUTPUT_DIR,
             CHUNKS_DIR_NAME);
    // 12: chunk_%d_%d.html
    int len_chunk_file_path =
        LEN_CHUNKS_DIR_PATH + snprintf(NULL, 0, "%d", INDEX_CURRENT_SHEET) +
        snprintf(NULL, 0, "%d", worksheet_callbackdata->num_of_chunks) + 13;
    char *CHUNK_FILE_PATH = (char *)XML_Char_malloc(len_chunk_file_path + 1);
    snprintf(CHUNK_FILE_PATH, len_chunk_file_path + 1, "%s/chunk_%d_%d.chunk",
             CHUNKS_DIR_PATH, INDEX_CURRENT_SHEET,
    ...
    snprintf(JSON_FILE_PATH, len_json_file_path + 1, "%s/chunk_%d_mc.json",
             CHUNKS_DIR_PATH, INDEX_CURRENT_SHEET);
    free(CHUNKS_DIR_PATH);
    ...
    worksheet_callbackdata->array_drawingids.drawing_ids =
        (char **)XML_Char_realloc(
            worksheet_callbackdata->array_drawingids.drawing_ids,
            worksheet_callbackdata->array_drawingids.length * sizeof(char *));
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "r:id") == 0) {
        worksheet_callbackdata->array_drawingids
            .drawing_ids[worksheet_callbackdata->array_drawingids.length - 1] =
            strdup(attrs[i + 1]);
      }
    }
    XML_SetElementHandler(xmlparser, NULL, worksheet_end_element);
...
void worksheet_end_element(void *callbackdata, const XML_Char *name) {
  if (XML_Char_icmp(name, "sheetData") == 0) {
    ...
    for (int index_col = 0;
         index_col < worksheet_callbackdata->array_cols.length; index_col++) {
...
void col_row_start_element(void *callbackdata, const XML_Char *name,
                           const XML_Char **attrs) {
  (void)attrs;
  if (XML_Char_icmp(name, "col") == 0) {
    ...
    if (worksheet_callbackdata->array_cols.length > 1) {
      _tmp_cols = (struct Col **)XML_Char_realloc(
          worksheet_callbackdata->array_cols.cols,
          worksheet_callbackdata->array_cols.length * sizeof(struct Col *));
      if (_tmp_cols) {
    ...
    worksheet_callbackdata->array_cols
        .cols[worksheet_callbackdata->array_cols.length - 1] =
        (struct Col *)XML_Char_malloc(sizeof(struct Col));
    worksheet_callbackdata->array_cols
        .cols[worksheet_callbackdata->array_cols.length - 1]
        ->isHidden = '0';
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "hidden") == 0) {
        worksheet_callbackdata->array_cols
            .cols[worksheet_callbackdata->array_cols.length - 1]
            ->isHidden = XML_Char_icmp(attrs[i + 1], "true") == 0 ? '1' : '0';
      } else if (XML_Char_icmp(attrs[i], "min") == 0) {
        worksheet_callbackdata->array_cols
            .cols[worksheet_callbackdata->array_cols.length - 1]
            ->min = (unsigned short)XML_Char_tol((char *)attrs[i + 1]);
      } else if (XML_Char_icmp(attrs[i], "max") == 0) {
        if (worksheet_callbackdata->has_dimension != '1')
          worksheet_callbackdata->end_col_number =
              (unsigned short)XML_Char_tol((char *)attrs[i + 1]);
        worksheet_callbackdata->array_cols
            .cols[worksheet_callbackdata->array_cols.length - 1]
            ->max = (unsigned short)XML_Char_tol((char *)attrs[i + 1]);
      } else if (XML_Char_icmp(attrs[i], "width") == 0) {
        worksheet_callbackdata->array_cols
            .cols[worksheet_callbackdata->array_cols.length - 1]
            ->width = strtof((char *)attrs[i + 1], NULL);
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "r") == 0) {
        ...
        worksheet_callbackdata->ROW_NUMBER =
            (unsigned short)XML_Char_tol(attrs[i + 1]);
        len_row_number =
            snprintf(NULL, 0, "%d", worksheet_callbackdata->ROW_NUMBER);
      } else if (XML_Char_icmp(attrs[i], "ht") == 0) {
        //<th style="height:px;"
        row_height_in_px = strtof((char *)attrs[i + 1], NULL) * (20 * 1.0 / 15);
      }
    }
    while (pre_row_number != 0 && worksheet_callbackdata->ROW_NUMBER != 0 &&
           pre_row_number < worksheet_callbackdata->ROW_NUMBER) {
      if (ftell(worksheet_file) >= CHUNK_SIZE_LIMIT) {
        ...
        snprintf(CHUNK_FILE_PATH, len_chunk_file_path + 1,
                 "%s/chunk_%d_%d.chunk", CHUNKS_DIR_PATH, INDEX_CURRENT_SHEET,
                 worksheet_callbackdata->num_of_chunks);
        free(CHUNKS_DIR_PATH);
        fclose(worksheet_file);
        worksheet_file = fopen(CHUNK_FILE_PATH, "w");
      ...
      for (int i = 1; i <= worksheet_callbackdata->end_col_number; i++) {
        ...
        snprintf(td, len_td_str + 1,
                 "<td id=\"%d_%s%d\" style=\"border-style:hidden;\"></td>",
                 INDEX_CURRENT_SHEET, col_name, pre_row_number);
    ...
    char TH_TAG[LEN_TH_TAG];
    snprintf(TH_TAG, LEN_TH_TAG, "<th style=\"height:%.2fpx;\">%d</th>",
             row_height_in_px, worksheet_callbackdata->ROW_NUMBER);
    fputs(TH_TAG, worksheet_file);
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "ref") == 0) {
        char **mergecell_range = (char **)XML_Char_malloc(2 * sizeof(char *));
        int count = 0;
        ...
        while ((token = strsep(&_tmp_range, ":"))) {
          ...
          mergecell_range[count] = (char *)XML_Char_malloc(len_token + 1);
          memcpy(mergecell_range[count], token, len_token + 1);
          mergecell_range[count][len_token] = '\0';
          count++;
        }
        // You can't free _tmp_range because its value can be changed by calls
        // to strsep(). The value of tofree consistently points to the start of
        ...
        // 30:  "":{colspan:"",rowspan:""}
        int len_mergecell_json_str =
            len_start_cell + len_colspan + len_rowspan + 30;
        char mergecell_json_str[len_mergecell_json_str + 1];
        snprintf(mergecell_json_str, len_mergecell_json_str + 1,
                 "\"%s\":{\"colspan\":\"%d\",\"rowspan\":\"%d\"}",
                 mergecell_range[0], colspan, rowspan);
...
void col_row_end_element(void *callbackdata, const XML_Char *name) {
  if (XML_Char_icmp(name, "col") == 0) {
    ...
    int len_row_number =
        snprintf(NULL, 0, "%d", worksheet_callbackdata->ROW_NUMBER);
    int len_index_sheet = snprintf(NULL, 0, "%d", INDEX_CURRENT_SHEET);
    while (START_CELL_IN_NUMBER_BY_ROW <=
           worksheet_callbackdata->end_col_number) {
...
void cell_start_element(void *callbackdata, const XML_Char *name,
  ...
  if (XML_Char_icmp(name, "c") == 0) {
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (XML_Char_icmp(attrs[i], "r") == 0) {
        worksheet_callbackdata->cell_name = (char *)XML_Char_realloc(
            worksheet_callbackdata->cell_name, 1 + XML_Char_len(attrs[i + 1]));
        memcpy(worksheet_callbackdata->cell_name, attrs[i + 1],
               1 + XML_Char_len(attrs[i + 1]));
        ...
        worksheet_callbackdata->type_content =
            (char *)XML_Char_realloc(worksheet_callbackdata->type_content,
                                     1 + XML_Char_len(attrs[i + 1]));
        memcpy(worksheet_callbackdata->type_content, attrs[i + 1],
               1 + XML_Char_len(attrs[i + 1]));
      }
    }
    int len_row_number =
    ...
    char wrapText = '\0';
    if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
            .isApplyAlignment == '1') {
      if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
              .alignment.horizontal != NULL) {
        int len_horizontal =
            XML_Char_len(array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                             .alignment.horizontal);
        horizontal = (char *)XML_Char_realloc(horizontal, 1 + len_horizontal);
        memcpy(horizontal,
               array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                   .alignment.horizontal,
               1 + len_horizontal);
        int len_vertical =
            XML_Char_len(array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                             .alignment.vertical);
        vertical = (char *)XML_Char_realloc(vertical, 1 + len_vertical);
        memcpy(vertical,
               array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                   .alignment.vertical,
               1 + len_vertical);
        wrapText = array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                       .alignment.isWrapText;
      }
    } else if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                   .isApplyAlignment == '0') {
      int id_cellXfs =
          array_cellXfs.Xfs[worksheet_callbackdata->index_style].xfId;
      if (array_cellStyleXfs.Xfs[id_cellXfs].alignment.horizontal != NULL) {
        int len_horizontal = XML_Char_len(
            array_cellStyleXfs.Xfs[id_cellXfs].alignment.horizontal);
        horizontal = (char *)XML_Char_realloc(horizontal, 1 + len_horizontal);
        memcpy(horizontal,
               array_cellStyleXfs.Xfs[id_cellXfs].alignment.horizontal,
               1 + len_horizontal);
        int len_vertical =
            XML_Char_len(array_cellStyleXfs.Xfs[id_cellXfs].alignment.vertical);
        vertical = (char *)XML_Char_realloc(vertical, 1 + len_vertical);
        memcpy(vertical, array_cellStyleXfs.Xfs[id_cellXfs].alignment.vertical,
               1 + len_vertical);
    ...
    if (horizontal != NULL) {
      if (XML_Char_icmp(horizontal, "center") == 0) {
        ...
        horizontal_style = (char *)XML_Char_realloc(horizontal_style, 18 + 1);
        snprintf(horizontal_style, 18 + 1, "text-align:center;");
      } else if (XML_Char_icmp(horizontal, "general") == 0) {
        // 16: text-align:left;
        horizontal_style = (char *)XML_Char_realloc(horizontal_style, 16 + 1);
        snprintf(horizontal_style, 16 + 1, "text-align:left;");
      } else {
        // 12: text-align:;
        horizontal_style = (char *)XML_Char_realloc(
            horizontal_style, XML_Char_len(horizontal) + 12 + 1);
        snprintf(horizontal_style, XML_Char_len(horizontal) + 12 + 1,
                 "text-align:%s;", horizontal);
      }
      free(horizontal);
    }
    if (vertical != NULL) {
      if (XML_Char_icmp(vertical, "center") == 0) {
        ...
        vertical_style = (char *)XML_Char_realloc(vertical_style, 22 + 1);
        snprintf(vertical_style, 22 + 1, "vertical-align:middle;");
      } else {
        // 16: vertical-align:;
        vertical_style = (char *)XML_Char_realloc(
            vertical_style, XML_Char_len(vertical) + 16 + 1);
        snprintf(vertical_style, XML_Char_len(vertical) + 16 + 1,
                 "vertical-align:%s;", vertical);
      }
      free(vertical);
    }
    if (wrapText == '1') {
      // 19: white-space:normal;
      wraptext_style = (char *)XML_Char_realloc(wraptext_style, 19 + 1);
    ...
    int border_id = -1;
    if (array_cellXfs.Xfs[worksheet_callbackdata->index_style].isApplyBorder ==
        '1') {
      border_id =
          array_cellXfs.Xfs[worksheet_callbackdata->index_style].borderId;
    } else if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                   .isApplyBorder == '0') {
      int id_cellXfs =
          array_cellXfs.Xfs[worksheet_callbackdata->index_style].xfId;
    ...
    if (border_id != -1) {
      ...
      if (array_borders.borders[border_id].bottom.style != NULL) {
        ...
        border_bottom = (char *)XML_Char_realloc(border_bottom, 27 + 1);
        snprintf(border_bottom, 27 + 1, "border-bottom-style:solid;");
        // TODO: If border_bottom == 'medium', then set border_width to 2px.
      } else {
        border_bottom = (char *)XML_Char_realloc(border_bottom, 28 + 1);
        snprintf(border_bottom, 28 + 1, "border-bottom-style:hidden;");
      }
    }
    int len_borders = XML_Char_len(border_left) + XML_Char_len(border_right) +
                      XML_Char_len(border_top) + XML_Char_len(border_bottom);
    border_style = (char *)XML_Char_realloc(border_style, len_borders + 1);
    snprintf(border_style, len_borders + 1, "%s%s%s%s", border_left,
    ...
    if (array_cellXfs.Xfs[worksheet_callbackdata->index_style].isApplyFont ==
        '1') {
    ...
    if (array_fonts.fonts[font_id].name != NULL) {
      // 12: "font-family:" | 1: ';'
      const int LEN_FONT_NAME =
          14 + XML_Char_len(array_fonts.fonts[font_id].name); // ex
      char font_name[LEN_FONT_NAME];
      snprintf(font_name, LEN_FONT_NAME, "font-family:%s;",
               array_fonts.fonts[font_id].name);
      font_style = strdup(font_name);
      if (array_fonts.fonts[font_id].sz != 0.0) {
        // 13: "font-size:px;" + 1: '\0' = 14
        const int LEN_FONT_SIZE =
            snprintf(NULL, 0, "%.2f", array_fonts.fonts[font_id].sz) + 14;
        char font_size[LEN_FONT_SIZE];
        snprintf(font_size, LEN_FONT_SIZE, "font-size:%.2fpx;",
                 array_fonts.fonts[font_id].sz);
      ...
      if (array_fonts.fonts[font_id].color.rgb != NULL) {
        const int LEN_FONT_COLOR_RGB =
            8 + XML_Char_len(array_fonts.fonts[font_id].color.rgb);
        char font_color_rgb[LEN_FONT_COLOR_RGB];
        // 6: "color:" | 1: ';'
        snprintf(font_color_rgb, LEN_FONT_COLOR_RGB, "color:%s;",
                 array_fonts.fonts[font_id].color.rgb);
        char *tmp_font_style = strdup(font_style);
        free(font_style);
        font_style = concat(tmp_font_style, font_color_rgb);
        free(tmp_font_style);
      }
      if (array_fonts.fonts[font_id].underline != NULL &&
          XML_Char_icmp(array_fonts.fonts[font_id].underline, "none") != 0) {
        ...
        free(tmp_font_style);
        if (XML_Char_icmp(array_fonts.fonts[font_id].underline, "single") ==
            0) {
          ...
          free(tmp_font_style);
        } else if (XML_Char_icmp(array_fonts.fonts[font_id].underline,
                                 "double") == 0) {
          const int LEN_FONT_TEXT_DECORATION_STYLE = 30;
          char font_text_decoration_style[LEN_FONT_TEXT_DECORATION_STYLE];
      ...
      unsigned short fill_id =
          array_cellXfs.Xfs[worksheet_callbackdata->index_style].fillId;
      int LEN_FILL_FGCOLOR_RGB = 0;
      if (array_fills.fills[fill_id].patternFill.fgColor.rgb != NULL) {
        // 18: "background-color:;"
        LEN_FILL_FGCOLOR_RGB =
            18 +
            XML_Char_len(array_fills.fills[fill_id].patternFill.fgColor.rgb);
        fill_style =
            (char *)XML_Char_realloc(fill_style, LEN_FILL_FGCOLOR_RGB + 1);
        snprintf(fill_style, LEN_FILL_FGCOLOR_RGB + 1, "background-color:%s;",
                 array_fills.fills[fill_id].patternFill.fgColor.rgb);
      } else {
        fill_style = (char *)calloc(1, sizeof(char));
      }
      if (horizontal_style == NULL) {
        horizontal_style = (char *)calloc(1, sizeof(char));
      }
      if (vertical_style == NULL) {
        vertical_style = (char *)calloc(1, sizeof(char));
      }
      if (wraptext_style == NULL) {
        wraptext_style = (char *)calloc(1, sizeof(char));
      }
      if (border_style == NULL) {
        border_style = (char *)calloc(1, sizeof(char));
      }
      if (font_style == NULL) {
        font_style = (char *)calloc(1, sizeof(char));
      }
      int len_styles =
          XML_Char_len(horizontal_style) + XML_Char_len(vertical_style) +
          XML_Char_len(wraptext_style) + XML_Char_len(border_style) +
          XML_Char_len(font_style) + LEN_FILL_FGCOLOR_RGB;
      styles = (char *)XML_Char_realloc(styles, len_styles + 1);
      snprintf(styles, len_styles + 1, "%s%s%s%s%s%s", horizontal_style,
               vertical_style, wraptext_style, border_style, font_style,
               fill_style);
      ...
      TD_TAG = (char *)XML_Char_realloc(TD_TAG, len_td_tag + 1);
      snprintf(TD_TAG, len_td_tag + 1, "<td id=\"%d_%s\" style=\"%s\">",
               INDEX_CURRENT_SHEET, worksheet_callbackdata->cell_name, styles);
      fputs(TD_TAG, worksheet_file);
      START_CELL_IN_NUMBER_BY_ROW++;
      free(styles);
      free(TD_TAG);
      XML_SetElementHandler(xmlparser, cell_item_start_element,
                            cell_end_element);
    }
  }
}
  ...
  if (XML_Char_icmp(name, "v") == 0) {
    ...
    if (worksheet_callbackdata->worksheet_content != NULL &&
      ...
      if (XML_Char_icmp(worksheet_callbackdata->type_content, "s") == 0) {
        ...
        int index_sharedStrings_current =
            (int)XML_Char_tol(worksheet_callbackdata->worksheet_content);
        unsigned long start_pos =
            sharedStrings_position.positions[index_sharedStrings_current];
        unsigned long end_pos = -1;
        if (index_sharedStrings_current + 1 <= len_pos_arr) {
          end_pos =
              sharedStrings_position.positions[index_sharedStrings_current + 1];
        }
        fseek(sharedStrings_file, start_pos, SEEK_SET);
        char c;
        while ((c = fgetc(sharedStrings_file)) != EOF) {
          fputc(c, worksheet_file);
      ...
      } else if (XML_Char_icmp(worksheet_callbackdata->type_content, "n") ==
                 0) {
        unsigned short numFmt_id = -1;
        if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                .isApplyNumberFormat == '1') {
          numFmt_id =
              array_cellXfs.Xfs[worksheet_callbackdata->index_style].numFmtId;
        } else if (array_cellXfs.Xfs[worksheet_callbackdata->index_style]
                       .isApplyNumberFormat == '0') {
          int id_cellXfs =
              array_cellXfs.Xfs[worksheet_callbackdata->index_style].xfId;
          numFmt_id = array_cellStyleXfs.Xfs[id_cellXfs].numFmtId;
        }
        int index_numFmt;
        for (index_numFmt = 0; index_numFmt < array_numfmts.length;
             index_numFmt++) {
          int numFmtIdInt =
              XML_Char_tol(array_numfmts.numfmts[index_numFmt].numFmtId);
          if (numFmtIdInt == numFmt_id) {
            break;
          }
        }
        if (XML_Char_icmp(array_numfmts.numfmts[index_numFmt].formatCode,
                          "General") == 0) {
          fputs("<span>", worksheet_file);
        } else {
          // <span class="n" data-format-code="">
          int len_span_html =
              XML_Char_len(array_numfmts.numfmts[index_numFmt].formatCode) + 36;
          char *span_html = (char *)XML_Char_malloc(len_span_html + 1);
          snprintf(span_html, len_span_html + 1,
                   "<span class=\"n\" data-format-code=\"%s\">",
                   array_numfmts.numfmts[index_numFmt].formatCode);
          fputs(span_html, worksheet_file);
          free(span_html);
        }
        fputs(worksheet_callbackdata->worksheet_content, worksheet_file);
        fputs("</span>", worksheet_file);
      } else {
        fputs("<span>", worksheet_file);
        fputs(worksheet_callbackdata->worksheet_content, worksheet_file);
    ...
    XML_SetElementHandler(xmlparser, cell_item_start_element,
                          cell_item_end_element);
    XML_SetCharacterDataHandler(xmlparser, NULL);
  }
}

void worksheet_content_handler(void *callbackdata, const XML_Char *buf,
                               int len) {
  if (len == 0) {
    return;
  }
  // The character data handler may be called multiple times with partial
  // character data for a single element.
  struct WorkSheet *worksheet_callbackdata = (struct WorkSheet *)callbackdata;
  if (worksheet_callbackdata->worksheet_content == NULL &&
      worksheet_callbackdata->len_worksheet_content == 0) {
    if ((worksheet_callbackdata->worksheet_content =
             (XML_Char *)XML_Char_realloc(
                 worksheet_callbackdata->worksheet_content, len + 1)) == NULL) {
      return;
    }
    ...
    if ((worksheet_callbackdata->worksheet_content = (XML_Char *)
             XML_Char_realloc(worksheet_callbackdata->worksheet_content,
                              len_worksheet_content + len)) == NULL) {
      return;
    }
    memcpy(worksheet_callbackdata->worksheet_content + len_worksheet_content -
               1,
           buf, len);
    worksheet_callbackdata->worksheet_content[len_worksheet_content + len - 1] =
        '\0';
