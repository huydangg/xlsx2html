  ...
  if (strcmp(name, "sheets") == 0) {
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_numfmts.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_numfmts.numfmts = calloc(array_numfmts.length, sizeof(struct NumFMT));
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_fonts.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_fonts.fonts = calloc(array_fonts.length, sizeof(struct Font));
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_fills.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_fills.fills = calloc(array_fills.length, sizeof(struct Fill));
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_borders.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_borders.borders = calloc(array_borders.length, sizeof(struct BorderCell));
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_cellStyleXfs.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_cellStyleXfs.Xfs = calloc(array_cellStyleXfs.length, sizeof(struct Xf));
      }
    }
    ...
    for (int i = 0; attrs[i]; i += 2) {
      if (strcmp(attrs[i], "count") == 0) {
        array_cellXfs.length = (unsigned short int)strtol((char *)attrs[i + 1], NULL, 10);
	array_cellXfs.Xfs = calloc(array_cellXfs.length, sizeof(struct Xf));
      }
    }
...
void sheet_main_start_element(void *userData, const XML_Char *name, const XML_Char **attrs) {
  if (strcmp(name, "sheet") == 0){
    ...
    count_sheet++;
    if (count_sheet > 1) {
      _tmp_sheets_callbackdata = realloc(array_sheets.sheets, sizeof(struct Sheet *) * count_sheet);
    ...
    array_sheets.sheets[count_sheet - 1] = malloc(sizeof(struct Sheet));
    for(int i = 0; attrs[i]; i += 2){
      if(strcmp(attrs[i], "state") == 0){
        array_sheets.sheets[count_sheet - 1]->isHidden = strcmp(attrs[i + 1], "hidden") == 0 ? '1' : '0';
      }
      if (strcmp(attrs[i], "name") == 0){
	array_sheets.sheets[count_sheet - 1]->name = malloc(sizeof(XML_Char) * (strlen(attrs[i + 1]) + 1));
	memcpy(array_sheets.sheets[count_sheet - 1]->name, attrs[i + 1], sizeof(XML_Char) * (strlen(attrs[i + 1]) + 1));
      }
      if (strcmp(attrs[i], "sheetId") == 0){
	array_sheets.sheets[count_sheet - 1]->sheet_id = malloc(sizeof(XML_Char) * (strlen(attrs[i + 1]) + 1));
	memcpy(array_sheets.sheets[count_sheet - 1]->sheet_id, attrs[i + 1], sizeof(XML_Char) * (strlen(attrs[i + 1]) + 1));
	char *pattern_name = "xl/worksheets/sheet.xml";
	array_sheets.sheets[count_sheet - 1]->path_name = insert_substr_to_str_at_pos(pattern_name, attrs[i + 1], 19);
      }
    } 
  }
